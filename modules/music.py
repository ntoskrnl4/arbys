...
detailed_help = {
	...
	"Description": "This command manages music related functionality within the bot. Music is available to several "
					"servers at once.",
...
class Song:
	def __init__(self, url: str, title: str = None, duration: int = None, requester: str = "<unknown requester>",
				noload: bool = False):
...
def get_target_voice_connection(object: Union[discord.Member, discord.Guild, discord.VoiceChannel, int]) -> Union[
	discord.VoiceClient, MusicException]:
	target = None
	...
	if isinstance(object, int):
		# seriously :/
		log.warning(
			"get_target_voice_channel() given a raw integer/ID; this probably should not happen but I'll try my best")
...
async def command(command: str, message: discord.Message):
	...
	if not voice_enable:
		await message.channel.send(
			"Sorry, but the internal Opus library required for voice support was not loaded for whatever reason, "
			"and music functionality will not work.")
		return
	...
	if not parts[1] in ["join", "add", "play", "skip", "pause", "volume", "queue", "info", "playing", "exit", "stop",
						"quit", "load", "remove"]:
	...
	async with access_lock:
		...
		if parts[1] == "join":
			...
			if connection is None:
				...
				if connection is None:
					...
					await message.channel.send(
						"Cannot join channel: Ambiguous/unknown target channel - please join the target voice channel")
					return
				else:
					# user's not in a channel but we are already in a channel
					await message.channel.send(
						"Cannot join channel: Already in a channel (also, next time please join the channel you are referring to first)")
					return
			if isinstance(connection, MusicException):
				...
				if user_state is None:
					await message.channel.send(
						"error: user voice status changed between two subsequent calls to retrieve user's channel")
					return
				
				channel_perms = user_state.channel.permissions_for(message.guild.me)
				if not channel_perms.connect:
					await message.channel.send(
						"Cannot join channel: insufficient permissions to connect to the channel")
					return
				if channel_perms.connect and not channel_perms.speak:
					await message.channel.send(
						"Refusing to join channel: insufficient permissions to send voice data in target channel")
					return
		...
		if parts[1] == "add":
			...
			await message.channel.send("Song added to end of queue:",
										embed=get_song_embed(song, queue_position=len(guild_queue[message.guild.id])))
			return
		...
		if parts[1] == "queue":
			try:
				...
				pass
			embed = discord.Embed(title="Upcoming Music Queue",
									description=get_queue_list(guild_queue.get(message.guild.id, [])),
		...
		if parts[1] == "info":
			...
			try:
				...
				if target_song is None:
					await message.channel.send(
						"Cannot get song information: no song is currently playing and no queue index specified")
					return
				else:
					await message.channel.send(
							embed=get_song_embed(
									target_song,
									is_next=True,
									is_paused=get_target_voice_connection(message.guild).is_paused()
							)
					)
					return
			...
			try:
				...
				await message.channel.send(
					f"Cannot get song info: invalid queue index (queue is shorter than index provided?)")
				return
		...
		if parts[1] in ["exit", "stop", "quit"]:
			...
			if not isinstance(vc, discord.VoiceClient):
				await message.channel.send(
					"Cannot disconnect from voice channel: not connected to any voice channel in this server")
				return
		...
		if parts[1] == "load":
			...
			if force_randomize and no_force_randomize:
				await message.channel.send(
					"Argument error: both --force-randomize and --no-force-randomize passed as arguments")
				return
			...
			try:
				...
				await message.channel.send(
					"Cannot load playlist: no playlist specified. See command help or source code for help.")
				return
			...
			try:
				...
				await message.channel.send(
					f"Cannot load playlist: unexpected exception loading playlist: {e.__class__.__name__}: {''.join(e.args)}\n\nThis exception has been logged.")
				return
