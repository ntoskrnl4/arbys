...
client.basic_help(title=f"{client.default_prefix}wa", desc="Performs a Wolfram|Alpha query and returns the results")
client.long_help(cmd="wa", mapping={
    "Usage": f"{client.default_prefix}wa",
    "Arguments": "Query to be evaluated",
    "Description": "The Wolfram|Alpha engine is a web-based computation engine that can perform a wide array of "
    ...
    "Note about results": "Because Wolfram|Alpha can return results about (seemingly) anything under the sun, "
...
async def WolframAlphaQuery(command: str, message: discord.Message):
    ...
    if (time.time()-last_run_time) < 15:
        await message.channel.send(f"{message.author.mention} Rate limited: please wait {15-(time.time()-last_run_time):.1f} seconds")
        return
    ...
    async with http_client.get("https://api.wolframalpha.com/v2/query", params=request_params) as _results:
        results = await _results.json(content_type="text/plain")
        query_url = _results.url.human_repr().replace(wa_key, "[key]")
    results = results['queryresult']
    if debug: await message.channel.send(f"`{query_url}`")
    ...
    if not (results['success'] or results['error']):
        ...
        embed = embed.add_field(name="Some tips for asking W|A", value="Try different phrasings of your query\n"
                                                                       ...
                                                                       "Double-check your spelling and use English")
        await message.channel.send(embed=embed)
        return
