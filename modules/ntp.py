...
async def get_ntp_stats(command: str, message: discord.Message):
	try:
		...
		precision = 2**int([x for x in parts if x.startswith("precision=")][0][10:])
	...
	if not alt:
		embed = embed.add_field(name="Sync Status", value=f"`{sync_status}")
		embed = embed.add_field(name="System Peer RefID", value=f"{refid}")
		embed = embed.add_field(name="System Stratum", value=stratum)
		embed = embed.add_field(name="Offset", value=f"{offset} ms")
		embed = embed.add_field(name="Jitter", value=f"{jitter} ms")
		embed = embed.add_field(name="Frequency Drift", value=f"{freq} ppm")
		embed = embed.add_field(name="Timing Precision", value=f"{precision*1000:.2f} ms")
