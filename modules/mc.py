...
class MinecraftServer:
	...
	def lookup(address):
		...
		if ":" in address:
			parts = address.split(":")
			if len(parts) > 2:
				log.warn(f"Invalid address {address}")
				raise RuntimeError("Invalid address, ping @ntoskrnl")
			host = parts[0]
			port = int(parts[1])
		if port is None:
			port = 25565
...
async def mc(command: str, message: discord.Message):
	async with message.author.typing():
		...
		online_players = "\n".join(players)  # Would do this inside the f-string
		# below... but you can't have escape
		...
		buffer = io.BytesIO(base64.b64decode(status.favicon.split(",")[1]))
		
		embed = discord.Embed(color=0x3D69DE,
								title="Young Hams unofficial minecraft server",
								description=f"IP: `{SERVER_URL}`\n")
		embed.set_thumbnail(url="attachment://icon.png")
		
		# embed = embed.add_field(name="Latency", value=f"{latency} ms")
		embed = embed.add_field(name="Version", value=f"{version}", inline=False)
		embed = embed.add_field(name="Players Online",
								value=f"{onlineplayercount}/{maxplayercount}\n{online_players}",
								inline=False)
